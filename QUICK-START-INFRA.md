# Optional — Quick start: extra infrastructure walkthrough

> **Status: not canonical.** This file is **optional** verification only. It **overlaps** the same ground as **[`docs/README.md`](docs/README.md)** (index) → **[`docs/INFRASTRUCTURE-ONBOARDING.md`](docs/INFRASTRUCTURE-ONBOARDING.md)** (story + order) → **[`docs/DEPLOYMENT-ORDER.md`](docs/DEPLOYMENT-ORDER.md)** (targets) → **`./spinup.sh`**. If you open Quick Start **and** those in parallel, you will not know which one is “truth”—so pick **onboarding + deployment order + spinup** first, and only open this file when you want extra checklists. **Do not start here** for Week 4; start at the docs index row for AWS EKS.

> **When you are done spending money on AWS:** From the repo root run **`./teardown.sh`**, choose `aws`, enter `dev` or `prod`, type `yes`, and wait for destroy to finish. Do not leave EKS, RDS, Redis, and MQ running overnight.
...
**Plain English:** What `spinup.sh` does step by step is spelled out in **[`docs/INFRASTRUCTURE-ONBOARDING.md`](docs/INFRASTRUCTURE-ONBOARDING.md)**. Concrete “we hit this bug during bring-up” lessons (Redis TLS, Secrets Manager, ESO/IRSA, CI→ECR, etc.) are folded into **[`TROUBLESHOOTING.md`](../TROUBLESHOOTING.md#spinup-and-eks-infra-lessons)** so you have one ops surface, not a second changelog file.
...
**Done here? → Return to [`LEARNING-PATH.md`](LEARNING-PATH.md) and continue Week 4** — then prefer the **canonical** trio ([`docs/README.md`](docs/README.md) → onboarding → deployment order) over this optional file.
