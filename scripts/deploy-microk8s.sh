...
case "$OS" in
  MINGW*|MSYS*|CYGWIN*)
    die "Native Windows shells are not supported (OS reports: ${OS}). Use WSL2 + Ubuntu: install Docker and MicroK8s inside WSL, open bash there, clone the repo, run ./scripts/deploy-microk8s.sh — see https://osomudeya.gumroad.com/l/payflow (MicroK8s / Platforms)."
    ;;
esac
...
case "$OS" in
  ...
  Linux)
    ...
    ;;
  *)
    die "Unsupported OS: ${OS}. Supported: macOS (Darwin), Linux (including WSL2). Windows: use WSL2 — see script header and https://osomudeya.gumroad.com/l/payflow (MicroK8s guide)."
    ;;
esac
