...
See the [MicroK8s deployment guide](https://osomudeya.gumroad.com/l/payflow) (*Building PayFlow* — full docs bundle, including `docs/microk8s-deployment.md`).
...
Add `www.payflow.local` and `api.payflow.local` to `/etc/hosts` so you can use the local Ingress without port-forwarding. Run once on your Mac after deploying with the local overlay. See [Access Application](https://osomudeya.gumroad.com/l/payflow) in the MicroK8s deployment guide (*Building PayFlow*).
...
Start stopped Multipass worker VMs so NotReady nodes become Ready again. See the [MicroK8s deployment guide](https://osomudeya.gumroad.com/l/payflow) — troubleshooting section for NotReady Multipass workers.
