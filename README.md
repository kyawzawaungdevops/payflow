> **Orientation (read this block before the architecture diagram).** PayFlow Wallet is a real fintech system you install on your own machine, fix failures, and learn to explain in a job interview, not a tutorial you watch while someone else types. You run services that look like production (an API gateway, auth, wallets, money movement, a message queue, PostgreSQL, Redis) and you put them on **Kubernetes** on your laptop first, then optionally on **AWS** with **Terraform** (the tool that declares VPCs, clusters, databases, and secrets in code so environments can be recreated).

> If you are wondering whether DevOps is still worth learning with AI everywhere? yes!!!. AI cannot replace an engineer who can debug a broken cluster. when alerts fire and the runbook does not match reality. This project is how you practice becoming that person: you read logs, fix failing pods, and redeploy until the platform stays up.

> **Your next step:** read **[`START-HERE.md`](START-HERE.md)** once (three files to trust), then open **[`LEARNING-PATH.md`](LEARNING-PATH.md)**, complete **Day 0**, and start **Week 1**. Nothing else yet.

> **Learning docs:** Follow [`LEARNING-PATH.md`](LEARNING-PATH.md) and [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md). Deploy and architecture depth live under **`docs/`**—see the index at [`docs/README.md`](docs/README.md). Optional narrative companion: [*Building PayFlow*](https://osomudeya.gumroad.com/l/payflow); if anything disagrees with this repo, **the code and these markdown files win**.
...
**Manual MicroK8s:** `microk8s enable dns storage registry ingress metrics-server`, kubeconfig, then `kubectl apply -k k8s/overlays/local`—see the [MicroK8s deployment guide](https://osomudeya.gumroad.com/l/payflow).
...
**Repo root:** Open this repository’s top-level folder in your editor (File → Open Folder in VS Code), then open a terminal there (Terminal → New Terminal). All commands below assume your shell’s working directory is the root—the folder that contains `README.md`.
...
**Terraform secrets:** The files under `terraform/aws/spoke-vpc-eks/`, `terraform/aws/managed-services/`, and `terraform/aws/finops/` ship with **placeholder** values (`REPLACE_WITH_*`). Edit them (or use `-var` / `TF_VAR_*`) with **unique** strong passwords and a **real** budget-alert inbox before you apply. If you ever applied a stack using **leaked, reused, or default** passwords from any bundle, **rotate** RDS, Amazon MQ, and app secrets in **AWS Secrets Manager** (and JWT signing material) in that account immediately—then invalidate client tokens if needed.

**Stop AWS/Azure spend when you are done:** from the repo root run **`./teardown.sh`** (choose cloud and `dev`/`prod`, confirm with `yes`). This is the **only** supported script for tearing down Terraform-managed cloud infrastructure in this repo.
...
|----------|-------------|
| **[Contributing & attribution](CONTRIBUTING.md)** | **Buyers / portfolio:** credit and bundle hygiene; optional upstream PRs |
...
| [MicroK8s](https://osomudeya.gumroad.com/l/payflow) | Local Kubernetes deploy and troubleshooting (*Building PayFlow*) |
...
**Continue learning:** **[`LEARNING-PATH.md`](LEARNING-PATH.md)** (Day 0 → weeks). This README is **orientation + environments**; the path file tells you what to do next.
